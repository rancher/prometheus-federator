...
#   TAG            - prometheus-federator release tag, used in PR title (required)
...
SOURCE_REPO="${SOURCE_REPO:-rancher/prometheus-federator}"

# Build the combined version string: {charts_version}+up{tag_without_v}
TAG_NO_V="${TAG#v}"
...
PR_TITLE="[${TARGET_BRANCH}] ${CHART_NAME} ${COMBINED_VERSION} ${ACTION}"
PR_BODY="Automated PR to update \`${CHART_NAME}\` chart from [${SOURCE_REPO}](https://github.com/${SOURCE_REPO}/releases/tag/${TAG}) release \`${TAG}\`."
